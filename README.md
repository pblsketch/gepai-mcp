...
또는 `~/.codex/config.toml`에 추가 (IDE 확장: ⚙ 메뉴 → **MCP settings → Open config.toml**). CLI·IDE 확장·데스크톱 앱(Windows·macOS)이 모두 이 설정 파일 하나를 공유하므로 한 번 등록하면 어디서든 쓸 수 있습니다:
...
| Codex (CLI·IDE·데스크톱 앱) | `~/.codex/config.toml`에 `[mcp_servers.gepai]` + `url = "https://gepai-mcp.vercel.app"` — 모든 Codex 제품이 이 파일을 공유 |
